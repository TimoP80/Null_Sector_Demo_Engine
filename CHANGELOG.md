...
- **AI Shader Generator self-healing** — when a fresh generation compiles but renders a degenerate frame (never drew / uniform / time-only / near-black), the generator re-asks the provider up to a configurable cap (default 1) with the probe's structured diagnosis riding along in the repair prompt; a repair's own result never re-triggers the chain. The status line shows the attempt counter and the final request count ("auto-repaired 1x - 2 requests"), and the whole auto path plus the vision input are togglable in Provider settings. Covered by new smoke assertions.
- **Multi-modal repair** — when enabled, the captured failing frame is PNG-encoded (via miniz) and attached to the repair request as a vision input (Chat Completions `image_url` / Responses API `input_image` parts), so image-capable OpenAI-compatible models see the actual broken frame instead of only a text description.
- **Static spatial linter** — fresh generations append an advisory source-inspection hint to the prompt when the current source has no per-pixel input (no `vUV` / `gl_FragCoord` / texture lookup), nudging the model before the first render.
- **Shared RenderProbe + structured diagnosis** — the engine gains one `probeRender()` utility (`src/engine/renderprobe.{hpp,cpp}`) that renders a program into an offscreen target at several instants, reads the pixels back, and classifies the output (never-drew / uniform / time-only / near-black) with a human-readable diagnosis; it saves and restores the caller's FBO/viewport/pack state. The AI generator, the Shader Lab, and the new shader preflight all use it - the three duplicated 64×64 readbacks are gone.
- **Shader Lab degenerate-output warning** — typography shaders run the shared RenderProbe after a successful compile: a source that renders as one uniform color (e.g. a glyph mask accidentally removed), never writes the output, or collapses to near-black is flagged with an amber warning carrying the probe's diagnosis instead of being accepted silently.
- **CI render check** — `--check-shaders --render` renders every self-contained content shader (no sampler uniforms, no custom vertex varyings) into a 16:9 offscreen target at two instants with the standard uniform set plus a plausible shared NullBlock (camera/beat/audio) and flags any that never drew, render one solid color, or go near-black - catching a flat shader before it ships. Intentional solid frames (fade/flash overlays, diag_* patterns) and pipeline-pass shaders are skipped; currently 20/20 content shaders pass.
...
- **Editor project packaging shipped the editor binary** — File > "Package Project…" copied the *running* executable (`ns_editor.exe`, which forces `--editor` and carries the full authoring layer) into the distribution ZIP as `<project>.exe`. The package now resolves the runtime-only engine player `ns_demo` that CMake builds next to the editor and ships that instead, with a clear error message when the player is not found alongside the editor.
